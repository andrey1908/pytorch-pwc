...
if __name__ == '__main__':
    ...
    firstImage = PIL.Image.open(args.first_image_file)
    secondImage = PIL.Image.open(args.second_image_file)

    assert(firstImage.mode == 'RGB')
    assert(secondImage.mode == 'RGB')

    tenFirst = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(firstImage)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
    tenSecond = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(secondImage)[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
